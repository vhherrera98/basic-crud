...
export function TaskForm({ data: outsideData, path: pathname }: IProps) {
  ...
  const onSave: SubmitHandler<IFormInput> = async (data) => {
    console.log(`${pathname}/tasks`)

    try {
      const response = await fetch(`${pathname}/tasks`, {
        method: 'POST',
        mode: 'cors',
        headers: {
  ...
  const onPut: SubmitHandler<IFormInput> = async (data) => {
    ...
    const response = await fetch(`${pathname}/tasks/${data.id}`, {
      method: 'PUT',
      mode: 'cors',
      headers: {
        'Content-Type': 'application/json'
